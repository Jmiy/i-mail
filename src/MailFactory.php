...
//        }
        ...
        //set mailer Queue
        if ($this->app->has(DriverFactory::class)) {
            $mailer->setQueue($this->app->get(DriverFactory::class));
        }
    ...
    public function createTransport(array $config)
    {
        ...
        $transport = $config['transport'] ?? $this->app->get(ConfigInterface::class)->get('mail.driver');
    ...
    protected function createSendmailTransport(array $config)
    {
        return new SendmailTransport(
            $config['path'] ?? $this->app->get(ConfigInterface::class)->get('mail.sendmail')
        );
    }
    ...
    protected function createPostmarkTransport(array $config)
    {
        return tap(new PostmarkTransport(
            $config['token'] ?? $this->app->get(ConfigInterface::class)->get('i_services.postmark.token')
        ), function ($transport) {
    ...
    public function getDefaultDriver()
    {
        ...
        $config = $this->app->get(ConfigInterface::class);
        return $config->get('mail.driver', $config->get('mail.default', 'smtp'));
    }
